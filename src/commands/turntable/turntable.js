...
import { musicSearch } from './_search';
...
export function turntable(orcabot, message) {
  ...
  if (!voiceChannel) {
    orcabot.reply(
      message,
      'You must be in a voice channel to use DJ commands!'
    );
    return;
  }
  ...
  if (connectedToVoice && botVoiceChannel === message.author.voiceChannel.id) {
    ...
    if (command === 'part') {
      ...
      if (!connectedToVoice.playing) {
        ...
        orcabot.reply(
          message,
          'Clear the queue before calling the `part` command!'
        );
      }
    }
      ...
      if (validURL) {
        ...
        if (!searchTerm.length) {
          ...
          if (!searchTerm.search(/^(sc:)/)) {
            ...
            ytImFeelingLucky(orcabot, message, searchTerm).then(
              searchResult => {
    ...
    if (!command.indexOf('search')) {
      ...
      musicSearch(orcabot, message, service, query).then(
        searchResult => {
          if (service === 'youtube') {
    ...
    if (connectedToVoice.playing) {
      ...
      if (command === 'shuffle') {
        ...
        queue.forEach(
          (element, index) => (queue[index] = shuffled[index]),
          this
        );
