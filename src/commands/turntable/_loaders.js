...
import { loader, player } from './_utils';
...
export function soundcloud(orcabot, message, songURL, validURL, queue) {
  ...
  if (validURL) {
    const requestURL = `http://api.soundcloud.com/resolve.json?url=${songURL}&client_id=${
      keys.scClientID
    }`;
    ...
    request(requestURL, (error, response, body) => {
      if (!error && response.statusCode === 200) {
        ...
        if (kind === 'track') {
          ...
          orcabot.reply(
            message,
            `Added \`${title}\` by \`${artist}\` to the queue!`
          );
          ...
          orcabot.reply(
            message,
            `Added playlist \`${playlistTitle}\` to the queue!`
          );
        }
    ...
    const requestURL = `http://api.soundcloud.com/tracks.json?q=${searchTerm}&client_id=${
      keys.scClientID
    }`;
