      ...
      (err, similarArtists) => {
        if (err) {
          ...
          for (let i = 0; i < numSimilarArtists; i++) {
            const { name, match } = similarArtists.artist[i];
    ...
    lfm.artist.getInfo(
      ...
      (err, artistInfo) => {
        ...
        const reply = entities
          .decode(artistInfo.bio.summary)
          .replace(/<(?:.|\n)*?>/gm, '')
          .replace(/\s+/g, ' ')
          .trim();

        // slice largest image from array of returned images
        let { image } = artistInfo;
...
const lfmdbPATH = 'cfg/lastfmdb.json';
let lfmdb = {};
    ...
    return new Promise((resolve, reject) => {
      ...
      lfm.user.getRecentTracks(
        ...
        (err, recentTracks) => {
          ...
          const { track: [{ loved }] } = recentTracks;
