...
export function textTranslate(orcabot, message) {
  ...
  if (message.content.search(/^(\.)(tr)( ).*?(:)/g) === 0) {
                ...
                if (!error) {
                  ...
                  translator.speak(params, (error, audiostream) => {
                    if (!error) {
                      ...
                      const req = request.post(uguu, (error, response, body) => {
                        if (error) {
                          orcabot.reply(message, 'There was an error with your translation.');
                          console.warn(`TRANSLATOR auto uguu request -- ${error}`);
                          console.warn(error);
                        } else {
                      ...
                      console.warn(`TRANSLATOR auto speak -- ${error}`);
                      console.warn(error);
                    }
                  });
                } else {
                  orcabot.reply(message, 'There was an error with your translation.');
                  console.warn(`TRANSLATOR auto translate -- ${error}`);
                  console.warn(error);
                }
              });
            } else {
              orcabot.reply(message, 'There was an error with your translation.');
              console.warn(`TRANSLATOR auto getLanguageNames -- ${error}`);
              console.warn(error);
            }
          });
        } else {
        ...
        if (!error) {
          ...
          translator.translate(params, (error, translatedText) => {
            if (!error) {
              ...
              orcabot.reply(message, 'There was an error with your translation.');
              console.warn(`TRANSLATOR not auto translate -- ${error}`);
              console.warn(error);
            }
          });
        } else {
    ...
    const found = message.content.match(detectedLanguageToEnglish);
    const inputText = message.content.substr(4).trim();
    params.to = 'en';
            ...
            translator.translate(params, (error, translatedText) => {
              if (!error) {
                ...
                translator.speak(params, (error, audiostream) => {
                  if (!error) {
                    const uguu = 'https://uguu.se/api.php?d=upload-tool';
                    const req = request.post(uguu, (error, resp, body) => {
                      if (error) {
                        orcabot.reply(message, 'There was an error with your translation.');
                        console.warn(`TRANSLATOR default uguu req -- ${error}`);
                        console.warn(error);
                      } else {
                        orcabot.reply(message, `Listen: ${body}`);
                      }
                    ...
                    console.warn(`TRANSLATOR default speak -- ${error}`);
                    console.warn(error);
                  }
                });
              } else {
                orcabot.reply(message, 'There was an error with your translation.');
                console.warn(`TRANSLATOR default translate -- ${error}`);
                console.warn(error);
              }
            });
          } else {
            orcabot.reply(message, 'There was an error with your translation.');
            console.warn(`TRANSLATOR default getLanguageNames -- ${error}`);
            console.warn(error);
          }
        });
      } else {
