...
/**
 * detect() gets language code of input text
 * @param {Object} languageParams - parameters for translation methods
 * @return {Object} updated parameters after detecting input language
 */
function detect(languageParams) {
  const params = languageParams;
  return new Promise((resolve, reject) => {
    translator.detect(params, (error, detectedLanguage) => {
      if (!error) {
...
/**
 * getLanguageNames() converts language codes to language names
 * @param {Object} languageParams - parameters for translation methods
 * @return {Array} array containing input and output languages
 */
function getLanguageNames(languageParams) {
  const params = languageParams;
  return new Promise((resolve, reject) => {
...
/**
 * translate() performs translation from input to output language
 * @param {Object} languageParams - parameters for translation methods
 * @param {Array} languageNames - array containing input and output languages
 * @param {Boolean} swap - determines if TTS language needs to be swapped
 * @return {Array} array containing response text, input and output languages
 * and translated text (if applicable)
 */
function translate(languageParams, languageNames, swap = false) {
  const params = languageParams;
  ...
  return new Promise((resolve, reject) => {
    translator.translate(params, (error, translatedText) => {
      if (!error) {
        ...
        if (swap) {
          // swap text for TTS depending on the input language
          params.text = translatedText;
          resolve([output, languageNames, translatedText]);
        } else {
          resolve([output, languageNames]);
        }
      } else {
        reject(error);
      }
...
/**
 * speak() streams spoken mp3 to uguu.se and provides media URL
 * @param {Object} languageParams - parameters for translation methods
 * @param {Array} languageNames - array containing input and output languages
 * @param {String} inputText - text to be spoken
 * @return {String} translation output text
 */
function speak(languageParams, languageNames, inputText) {
  const params = languageParams;
  const [inputLanguage, outputLanguage] = languageNames;
  return new Promise((resolve, reject) => {
...
/**
 * detectWrap() wraps the flow of promises from language detection down to translation output
 * @param {Object} languageParams - parameters for translation methods
 * @param {String} inputText - text to be spoken
 * @param {String} detectType - denotes whether detection is automatic or default (English)
 * @return {String} translation output text
 */
...
export function textTranslate(message) {
  ...
  if (!message.content.search(/^(\.)(tr)( ).*?(:)/g)) {
    // search for a colon after '.tr languagecode' in input command
    if (!message.content.search(/^(\.)(tr)( )(auto)(:).*?( )/g)) {
      // '.tr auto:languagecode '
      const found = message.content.match(/^(\.)(tr)( )(auto)(:).*?( )/g);
      ...
      return detectWrap(params, inputText, 'auto');
    } else if (!message.content.search(/(\.)(tr)( ).*?(:).*?( )/g)) {
      // '.tr languagecode:languagecode'
      const found = message.content.match(/(\.)(tr)( ).*?(:).*?( )/g);
      const str = found.toString().split(':'); // '.tr code', 'code '
    ...
    // gets language code for input text
    return detectWrap(params, inputText, 'default');
  }
