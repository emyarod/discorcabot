...
export function textTranslate(orcabot, message) {
  ...
  // TODO: add commands to list available languages
  let translateLanguageList;
  let speakLanguageList;
  translator.getLanguagesForTranslate((error, data) => {
    if (!error) {
      translateLanguageList = data;
      console.log(translateLanguageList);
    } else {
      console.warn(`TRANSLATOR getLanguagesForTranslate -- ${error}`);
      console.warn(error);
    }
  });

  translator.getLanguagesForSpeak((error, data) => {
    if (!error) {
      speakLanguageList = data;
      console.log(speakLanguageList);
    } else {
      console.warn(`TRANSLATOR getLanguagesForSpeak -- ${error}`);
      console.warn(error);
    }
  });
  ...
  if (message.content.search(/^(\.)(tr)( ).*?(:)/g) === 0) {
    // search for a colon after '.tr languagecode' in input command
    if (message.content.search(/^(\.)(tr)( )(auto)(:).*?( )/g) === 0) {
                ...
                if (!err) {
                  ...
                  translator.speak(params, (er, audiostream) => {
                    if (!er) {
                      ...
                      console.warn(`TRANSLATOR auto speak -- ${er}`);
                      console.warn(er);
                    }
                  });
                } else {
                  orcabot.reply(message, 'There was an error with your translation.');
                  console.warn(`TRANSLATOR auto translate -- ${err}`);
                  console.warn(err);
                }
              });
            } else {
              orcabot.reply(message, 'There was an error with your translation.');
              console.warn(`TRANSLATOR auto getLanguageNames -- ${errs}`);
              console.warn(errs);
            }
          });
        } else {
          orcabot.reply(message, 'There was an error with your translation.');
          console.warn(`TRANSLATOR auto detect -- ${error}`);
          console.warn(error);
        }
      });
    } else if (message.content.search(/(\.)(tr)( ).*?(:).*?( )/g) === 0) {
      ...
      translator.getLanguageNames(params, (error, languageNames) => {
        if (!error) {
          ...
          orcabot.reply(message, 'There was an error with your translation.');
          console.warn(`TRANSLATOR not auto getLanguageNames -- ${error}`);
          console.warn(error);
        }
      });
    }
  } else if (message.content.search(detectedLanguageToEnglish) === 0) {
      ...
      if (!error) {
        ...
        translator.getLanguageNames(params, (errs, languageNames) => {
          if (!errs) {
            ...
            translator.translate(params, (err, translatedText) => {
              if (!err) {
                const content = `${translatedText} (${inputLanguage} to ${outputLanguage})`;
                ...
                translator.speak(params, (er, audiostream) => {
                  if (!er) {
                    const uguu = 'https://uguu.se/api.php?d=upload-tool';
                    const req = request.post(uguu, (e, resp, body) => {
                      if (e) {
                        orcabot.reply(message, 'There was an error with your translation.');
                        console.warn(`TRANSLATOR default uguu req -- ${e}`);
                        console.warn(e);
                      } else {
                        orcabot.reply(message, `Listen: ${body}`);
                      }
                    ...
                    console.warn(`TRANSLATOR default speak -- ${er}`);
                    console.warn(er);
                  }
                });
              } else {
                orcabot.reply(message, 'There was an error with your translation.');
                console.warn(`TRANSLATOR default translate -- ${err}`);
                console.warn(err);
              }
            });
          } else {
            orcabot.reply(message, 'There was an error with your translation.');
            console.warn(`TRANSLATOR default getLanguageNames -- ${errs}`);
            console.warn(errs);
          }
        });
      } else {
        orcabot.reply(message, 'There was an error with your translation.');
        console.warn(`TRANSLATOR default detect -- ${error}`);
        console.warn(error);
      }
    });
  }
